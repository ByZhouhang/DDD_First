...
namespace ManageSystem
{
    public partial class FrmUserRightManage : Form
    {
        public FrmUserRightManage()
        {
            ...
            //多线程加载  用户列表
            MultiThreadingBindingUserData(1).ContinueWith(
                async (preTask) =>
                {

                    //异步加载  角色列表
                    await AsyncLoadData_RoleList();

                    //异步加载   权限列表
                    await AsyncLoadData_RightList();
        ...
        //所有的权限项  用于形成树形菜单
        List<RightDto> _allRightList;

        //角色对应权限的一级目录表
        List<RightDto> _firstMenuRightList;
        ...
        #endregion
        private Task MultiThreadingBindingUserData(int pageIndex)
        {
            //绑定数据源时会触发SelectionChanged
            dgvUserList.SelectionChanged -= dgvUserList_SelectionChanged;

            return Task.Factory.StartNew(() => LoadData(pageIndex))
        ...
        /// 装载权限列表  渲染TreeView控件
        /// </summary>
        private async Task AsyncLoadData_RightList()
        {
            var rightService = AutofacContainer.GetInstance().GetObject<IRightService>();
            //所有的权限项
            _allRightList = await rightService.GetRightList();

            _firstMenuRightList = _allRightList.Where(o => o.PID == 0).ToList();
            ...
            //查找一级节点  在数据中PID==0
            CreateChildNode(rootNode, 0, this._allRightList);
        ...
        {
            if (_firstMenuRightList == null) return;
